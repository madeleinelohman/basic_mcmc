...
ratesLike <- function(x, prob, type, z, v, t){
  ...
  if(type == 'kappa'){
    ugh <- sum(dbinom(v[[x]][,t],
                      z[[x]][,t],
                      (1-prob[x]) * rr[t] * Cr, log=T), na.rm=T)
    return(ugh)
  }
...
### Reporting rates
rr <- c(0.45,0.45,0.45,0.45,0.45,0.45,0.45,0.45,0.45,0.45,0.45,0.45,0.45,0.45,0.45,
        0.45,0.45,0.45,0.45,0.45,0.45,0.45,0.45,0.45,0.44,0.44,0.44,0.44,0.44,0.44,
        0.44,0.44,0.44,0.44,0.44,0.51,0.64,0.83,0.83,0.83,0.83,0.75,0.75,0.75,0.75,
        0.76,0.75,0.75,0.75,0.75,0.75,0.75,0.75,0.75,0.75,0.75,0.75)
### Crippling rate
Cr = 0.8
...
for(t in 1:(T-1)){
  for(i in 1:(S)){
    plot(epsilon.save[ind,i,t],type='l',ylim=c(-5,5), main=paste("site = ",i,"time = ", t))
...
for(t in 1:T){
  for(i in 1:(S)){
    plot(eta.save[ind,i,t],type='l', main=paste("eta: t = ", t, ", s = ",i))
